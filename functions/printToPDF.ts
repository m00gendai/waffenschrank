...
console.log(gun)
    ...
    <html>
      ...
      <div class="bodyContent">
        <h1>
          ${gun.manufacturer ? gun.manufacturer : ""} ${gun.model}
        </h1>
        <div class="imageContainer">
        ${imgs !== null ? `${imgs.map(img => {return `<div class="image" style="background-image: url(data:image/jpeg;base64,${img});"></div>`}).join("")}`: null}
        </div>
        <div class="tagContainer">
        ${gun.tags && gun.tags.length !== 0 ? gun.tags.map(tag => {return `<div class="tag">${tag}</div>`}).join("") : null}
        </div>
        <table>
            <tbody>
        ...
        <div class="remarkContainer">
            <div class="remarkContainerTitle">
                <strong>${gunRemarks[language]}</strong>
            </div>
            <div class="remarkContainerContent">${gun.remarks}</div>
        </div>
        </div>
