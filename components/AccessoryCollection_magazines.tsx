...
import { GunType, MenuVisibility, SortingTypes_Accessory_Magazine } from '../interfaces';
...
export default function AccessoryCollection_optics({navigation, route}){
  ...
  const { language, theme, magazinesFilterOn } = usePreferenceStore()
  const { mainMenuOpen, setHideBottomSheet } = useViewStore()
  const { setCurrentItem } = useItemStore()
  const [sortBy, setSortBy] = useState<SortingTypes_Accessory_Magazine>("alphabetical")

  const { data: settingsData } = useLiveQuery(
    db.select().from(schema.settings)
  )

  const { data: magazinesData } = useLiveQuery(
    db.select()
    .from(schema.magazineCollection)
    .where(
      and(
        or(
          like(schema.magazineCollection.designation, `%${searchQuery}%`),
          like(schema.magazineCollection.manufacturer, `%${searchQuery}%`)
        ),
      )
    )
    .orderBy(
      settingsData[0]?.sortOrder_Accessory_Magazine === "asc" ?
        sortBy === "alphabetical" ?
          asc((sql`COALESCE(NULLIF(${schema.magazineCollection.manufacturer}, ""), ${schema.magazineCollection.designation})`))
          : (sql`
            CASE
              WHEN NULLIF(${schema.magazineCollection[sortBy]}, "") IS NULL THEN NULL
              ELSE strftime('%s', ${schema.magazineCollection[sortBy]})
            END ASC NULLS LAST`)
        :
        sortBy === "alphabetical" ?
          desc((sql`COALESCE(NULLIF(${schema.magazineCollection.manufacturer}, ""), ${schema.magazineCollection.designation})`))
          : (sql`
            CASE
                WHEN NULLIF(${schema.magazineCollection[sortBy]}, "") IS NULL THEN NULL
                ELSE strftime('%s', ${schema.magazineCollection[sortBy]})
    ...
    [searchQuery, settingsData[0]?.sortOrder_Accessory_Magazine, sortBy]
  )
  ...
  async function handleSortBy(type: SortingTypes_Accessory_Magazine){
    setSortBy(type)
    await db.update(schema.settings).set({sortBy_Accessory_Magazine: type})
  }
  ...
  async function handleSortOrder(){
    ...
    const sortOrder = settings[0]?.sortOrder_Accessory_Magazine || "asc"
    await db.update(schema.settings).set({sortOrder_Accessory_Magazine: sortOrder === "asc" ? "desc" : "asc"})
  }
        
  async function handleDisplaySwitch(){
    const settings = await db.select().from(schema.settings)
    const displayStyle = settings[0]?.displayMode_Accessory_Magazine || "grid"
    await db.update(schema.settings).set({displayMode_Accessory_Magazine: displayStyle === "grid" ? "list" : "grid"})
  } 
  ...
  function handleFAB(){
    ...
    setHideBottomSheet(true)
      navigation.navigate("NewItem", {itemType: "Accessory_Magazine"})
  }
    ...
    <View style={{flex: 1, backgroundColor: "transparent"}}>
      <Appbar style={{width: "100%", display: "flex", flexDirection: "row", justifyContent: "space-between"}}>
        ...
        <View  style={{display: "flex", flexDirection: "row", justifyContent: "flex-end"}}>
          ...
          <Appbar.Action icon={settingsData[0]?.sortOrder_Accessory_Magazine === "asc" ? "arrow-up" : "arrow-down"} onPress={() => handleSortOrder()} />
        </View>
      </Appbar>
      ...
      {settingsData[0]?.displayMode_Accessory_Silencer === "grid" ? 
        ...
        <FlatList 
          ...
          style={{height: "100%", width: "100%", paddingTop: defaultViewPadding, paddingLeft: defaultViewPadding, paddingRight: defaultViewPadding, paddingBottom: 50}} 
           /*@ts-expect-error*/
           data={magazinesData.filter(gun => magazinesFilterOn ? tagData.filter(tag => tag.active).every(tag => gun.tags?.includes(tag.label)) : gun)} 
           /*@ts-expect-error*/
          renderItem={({item, index}) => <ItemCard item={item} itemType="Accessory_Magazine" />}                     
      ...
      <Animated.View style={[{position: "absolute", bottom: defaultBottomBarHeight+defaultViewPadding, right: 0, margin: 16, width: 56, height: 56, backgroundColor: "transparent", display: "flex", justifyContent: "center", alignItems: "center"}, magazinesData.length === 0 ? pulsate : null]}>
        <FAB
          icon="plus"
          onPress={()=>handleFAB()}
