...
import { GunType, MenuVisibility, SortingTypes_Accessory_Optic } from '../interfaces';
...
export default function AccessoryCollection_optics({navigation, route}){
  ...
  const [sortBy, setSortBy] = useState<SortingTypes_Accessory_Optic>("alphabetical")

  const { data: settingsData } = useLiveQuery(
    db.select().from(schema.settings)
  )

  const { data: opticsData } = useLiveQuery(
    db.select()
    .from(schema.opticsCollection)
    .where(
      and(
        or(
          like(schema.opticsCollection.designation, `%${searchQuery}%`),
          like(schema.opticsCollection.manufacturer, `%${searchQuery}%`)
        ),
      )
    )
    .orderBy(
      settingsData[0]?.sortOrder_Accessory_Optic === "asc" ?
        sortBy === "alphabetical" ?
          asc((sql`COALESCE(NULLIF(${schema.opticsCollection.manufacturer}, ""), ${schema.opticsCollection.designation})`))
          : (sql`
            CASE
              WHEN NULLIF(${schema.opticsCollection[sortBy]}, "") IS NULL THEN NULL
              ELSE strftime('%s', ${schema.opticsCollection[sortBy]})
            END ASC NULLS LAST`)
        :
        sortBy === "alphabetical" ?
          desc((sql`COALESCE(NULLIF(${schema.opticsCollection.manufacturer}, ""), ${schema.opticsCollection.designation})`))
          : (sql`
            CASE
                WHEN NULLIF(${schema.opticsCollection[sortBy]}, "") IS NULL THEN NULL
                ELSE strftime('%s', ${schema.opticsCollection[sortBy]})
    ...
    [searchQuery, settingsData[0]?.sortOrder_Accessory_Optic, sortBy]
  )
  ...
  async function handleSortBy(type: SortingTypes_Accessory_Optic){
    setSortBy(type)
    await db.update(schema.settings).set({sortBy_Accessory_Optic: type})
  }
  ...
  async function handleSortOrder(){
    ...
    const sortOrder = settings[0]?.sortOrder_Accessory_Optic || "asc"
    await db.update(schema.settings).set({sortOrder_Accessory_Optic: sortOrder === "asc" ? "desc" : "asc"})
  }
        
  async function handleDisplaySwitch(){
    const settings = await db.select().from(schema.settings)
    const displayStyle = settings[0]?.displayMode_Accessory_Optic || "grid"
    await db.update(schema.settings).set({displayMode_Accessory_Optic: displayStyle === "grid" ? "list" : "grid"})
  } 
  ...
  function handleFAB(){
    ...
    setHideBottomSheet(true)
      navigation.navigate("NewItem", {itemType: "Accessory_Optic"})
  }
    ...
    <View style={{flex: 1, backgroundColor: "transparent"}}>
      <Appbar style={{width: "100%", display: "flex", flexDirection: "row", justifyContent: "space-between"}}>
        ...
        <View  style={{display: "flex", flexDirection: "row", justifyContent: "flex-end"}}>
          ...
          <Appbar.Action icon={settingsData[0]?.displayMode_Accessory_Optic === "grid" ? "view-grid" : "format-list-bulleted-type"} onPress={handleDisplaySwitch} />
          <Menu
          ...
          <Appbar.Action icon={settingsData[0]?.sortOrder_Accessory_Optic === "asc" ? "arrow-up" : "arrow-down"} onPress={() => handleSortOrder()} />
        </View>
      </Appbar>
      ...
      {settingsData[0]?.displayMode_Accessory_Optic === "grid" ? 
        ...
        <FlatList 
          ...
          style={{height: "100%", width: "100%", paddingTop: defaultViewPadding, paddingLeft: defaultViewPadding, paddingRight: defaultViewPadding, paddingBottom: 50}} 
           /*@ts-expect-error*/
           data={opticsData.filter(gun => opticsFilterOn ? tagData.filter(tag => tag.active).every(tag => gun.tags?.includes(tag.label)) : gun)} 
           /*@ts-expect-error*/
          renderItem={({item, index}) => <ItemCard item={item} itemType="Accessory_Optic" />}                     
      ...
      <Animated.View style={[{position: "absolute", bottom: defaultBottomBarHeight+defaultViewPadding, right: 0, margin: 16, width: 56, height: 56, backgroundColor: "transparent", display: "flex", justifyContent: "center", alignItems: "center"}, opticsData.length === 0 ? pulsate : null]}>
        <FAB
          icon="plus"
          onPress={()=>handleFAB()}
